...
const StudentDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStudentData = async () => {
      try {
        ...
        const profileResponse = await axios.get(`${API}/api/auth/me`, { headers });
        const profileUser = profileResponse.data?.user || user;
        setStudentData(profileUser);

        const studentIdentifiers = [
          profileUser?.studentId, profileUser?.rollNumber,
          user?.studentId, user?.rollNumber, user?.id,
        ].filter(Boolean);
  ...
  // Get student roll number for AI prediction
  const studentRollNumber = studentData?.rollNumber || user?.rollNumber || '2024CS101';
              ...
              <CardContent className="p-6">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className={`text-xs font-medium mt-1 ${parseFloat(overallAttendance) >= 75 ? 'text-green-600' : 'text-red-600'}`}>
              ...
              <CardContent className="p-6">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">{studentData?.semester || 'N/A'} semester</p>
                  </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {attendanceData.map((data: any) => (
                <Card key={data._id} className="border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                  <CardContent className="p-5">
                    ...
                    <h4 className="font-semibold text-slate-900 mb-1 line-clamp-1">{data.subject}</h4>
                    <p className="text-xs text-slate-500">{data.attended} / {data.total} Classes Attended</p>
        ...
        <aside className="lg:col-span-4 space-y-6">
          <Card className="border-slate-200 shadow-sm">
            ...
            <CardContent className="pt-6 space-y-6">
              <TimelineCard type="class" title="Advanced Data Structures" time="09:00 AM - 10:30 AM" />
              <TimelineCard type="assignment" title="ML Lab Submission" time="Due today, 11:59 PM" urgent={true} />
              <TimelineCard type="event" title="Cloud Tech Seminar" time="02:00 PM - Auditorium" />
            </CardContent>
          </Card>
          ...
          <Card className="border-slate-200 shadow-sm bg-primary text-white">
            <CardContent className="p-6">
              ...
              <p className="text-blue-100 text-sm mb-4">View upcoming recruitment drives and manage your applications.</p>
              <Button variant="secondary" className="w-full bg-white text-primary hover:bg-slate-50 border-0"
